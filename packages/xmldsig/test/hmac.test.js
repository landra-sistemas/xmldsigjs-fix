...
require("./config.js");
(0, vitest_1.describe)('HMAC', () => {
    ['SHA-1', 'SHA-256', 'SHA-384', 'SHA-512'].forEach((hash) => {
        (0, vitest_1.it)(`hash:${hash}`, async () => {
            ...
            await signature.Sign({ name: 'HMAC' }, key, xmldsig.Parse(`<root><first id="id1"><foo>hello</foo></first></root>`), {
            ...
            vitest_1.assert.equal(signature2.XmlSignature.SignatureValue?.length, signatureValueSize / 8);
            vitest_1.assert.equal(si.SignatureMethod.HMACOutputLength, undefined);
            let digestMethod;
            switch (hash) {
                case 'SHA-1':
