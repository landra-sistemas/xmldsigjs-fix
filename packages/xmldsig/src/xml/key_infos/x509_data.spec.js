"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const vitest_1 = require("vitest");
require("../../../test/config.js");
const x509_data_js_1 = require("./x509_data.js");
(0, vitest_1.describe)('X509Data', () => {
    const xmlX509 = `<ds:X509Data xmlns:ds="http://www.w3.org/2000/09/xmldsig#"><ds:X509Certificate></ds:X509Certificate></ds:X509Data>`;
    (0, vitest_1.describe)('Load', () => {
        (0, vitest_1.it)('X509Certificate', () => {
            const x509data = x509_data_js_1.KeyInfoX509Data.LoadXml(xmlX509);
            vitest_1.assert.equal(x509data.Certificates.length, 1);
        });
        (0, vitest_1.it)('X509CRL', () => {
            const xml = `<ds:X509Data xmlns:ds="http://www.w3.org/2000/09/xmldsig#"><ds:X509CRL></ds:X509CRL></ds:X509Data>`;
            const x509data = x509_data_js_1.KeyInfoX509Data.LoadXml(xml);
            vitest_1.assert.equal(!!x509data.CRL, true);
        });
    });
    (0, vitest_1.it)('export key', async () => {
        const x509data = x509_data_js_1.KeyInfoX509Data.LoadXml(xmlX509);
        const key = await x509data.exportKey({ name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' });
        vitest_1.assert.equal(!!key, true);
        vitest_1.assert.equal(!!x509data.Key, true);
    });
});
